...
import { Card, Col, Progress, Row, Select, Spin, Statistic, Table, Tag } from 'antd';
...
const WeeklyRiskDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const weeklyTrendsResponse = await fetch('http://localhost:3000/overview/employee-weekly-trends');
        const weeklyTrendsData = await weeklyTrendsResponse.json();
        
        // Fetch risk assessment data
        const riskResponse = await fetch('http://localhost:3000/overview/risk-assessment');
        const riskAssessmentData = await riskResponse.json();
        
        // Fetch attendance data from the new endpoint
        const attendanceResponse = await fetch('http://localhost:3000/get-attendance');
  ...
  const teamColumns = [
    ...
    },
    {
      ...
      sorter: (a, b) => parseFloat(a.risk_score) - parseFloat(b.risk_score),
      render: (score) => (
        <Progress 
          percent={score * 10} 
          size="small" 
    ...
    },
    {
      ...
      render: (value) => (
        <span style={{ color: parseFloat(value) <= 0 ? '#f5222d' : '#52c41a' }}>
          {value > 0 ? '+' : ''}{value}%
          {parseFloat(value) === 0 ? null : value < 0 ? <ArrowDownOutlined /> : <ArrowUpOutlined />}
        </span>
      )
    },
    {
    ...
    const RecommendationCell = ({ metrics, riskScore, employeeName }) => {
      ...
      useEffect(() => {
        const fetchRecommendations = async () => {
          try {
            const response = await fetch("http://localhost:3000/api/performance/recommendations", {
              method: "POST",
            ...
            // Ensure we always set an array of recommendations
            const recommendations = data.recommendations || "";
            setRecs(typeof recommendations === 'string' 
              ? [recommendations] 
              : Array.isArray(recommendations) 
                ? recommendations 
                : ["Unable to parse recommendations."]);
          } catch (error) {
          ...
          <div className="weekly-changes">
            ...
            <div className="change-item">
              ...
              <span className={`change-value ${
                record.metrics.attendance?.attendance_rate === 0 ? 'critical' : 
                record.metrics.attendance?.attendance_rate < 35 ? 'critical' :
                record.metrics.attendance?.attendance_rate < 70 ? 'negative' : 'normal'
              }`}>
                {(record.metrics.attendance?.attendance_rate || 0).toFixed(1)}%
              </span>
            </div>
          </div>
        )
        ...
        <Row gutter={16} className="stats-row">
          ...
          <Col span={6}>
            <Statistic
              ...
              value={`${riskData.company_risk?.risk_breakdown?.retention_risk || 0} / ${riskData.company_risk?.total_teams || 0}`}
              valueStyle={{ color: '#cf1322' }}
              prefix={<WarningOutlined />}
            />
            <div className="stat-subtitle">Risk Score ≥ 6.5</div>
